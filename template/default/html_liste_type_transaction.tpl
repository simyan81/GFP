...
{* Initialise quelques variables *}
  ...
  {/if}
  {if !isset($title)}
    {assign var="title" value="Type de transaction"}
  {/if}
  {if !isset($placeholder)}
    {assign var="placeholder" value="Type de transaction"}
  {/if}
  {if !isset($disabled)}
...
<select {$id} {$name} {$title} {$placeholder}
        ...
        {if $extra <> ""} {$extra} {/if}
        autocomplete="off">
  {foreach $liste_type_transaction as $id => $ligne_type_transaction}
    <option value="{$ligne_type_transaction.id}"
      {if $ligne_type_transaction.id == $defaultvalue} selected {/if}
      {if $disablevalue == $ligne_type_transaction.id}
        disabled="disabled"
      {/if}
      {if $disablevalue2 == $ligne_type_transaction.id}
        disabled="disabled"
      {/if}
    >{$ligne_type_transaction.description}
    </option>
  {/foreach}
</select>
